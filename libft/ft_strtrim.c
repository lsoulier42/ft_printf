...
static int	is_charset_strtrim(char c, const char *set)
{
	int i;
...
static int	char_to_rmv(const char *s1, const char *set, size_t len)
{
	...
	j = 0;
	nb = 0;
	while (s1[i] && is_charset_strtrim(s1[i], set))
	{
		nb++;
		i++;
	}
	if (i != ft_strlen(s1))
	{
		while (len >= j + 1 && is_charset_strtrim(s1[len - j - 1], set))
		{
			nb++;
			j++;
		}
...
char		*ft_strtrim(const char *s1, const char *set)
{
	...
	if (s1)
	{
		...
		if (!(new = (char*)malloc(sizeof(char) * (char_to_copy + 1))))
			return (NULL);
		while (s1[start_index] && is_charset_strtrim(s1[start_index], set))
			start_index++;
